...
impl SubgraphEntity {
    ...
    pub fn write_ops(self) -> Vec<EntityOperation> {
        let mut ops = vec![];

        let manifest_id = format!("{}-manifest", self.id);
        ops.append(&mut self.manifest.write_ops(&manifest_id));
...
impl SubgraphManifestEntity {
    fn write_ops(self, id: &str) -> Vec<EntityOperation> {
        let mut ops = vec![];
        ...
        for (i, data_source) in self.data_sources.into_iter().enumerate() {
            ...
            ops.append(&mut data_source.write_ops(&data_source_id));
        ...
        ops.push(set_entity_op("SubgraphManifest", id, entity));
...
impl EthereumContractDataSourceEntity {
    fn write_ops(self, id: &str) -> Vec<EntityOperation> {
        ...
        ops.append(&mut self.source.write_ops(&source_id));

        let mapping_id = format!("{}-mapping", id);
        ops.append(&mut self.mapping.write_ops(&mapping_id));
        ...
        ops.push(set_entity_op("EthereumContractDataSource", id, entity));
...
impl EthereumContractSourceEntity {
    fn write_ops(self, id: &str) -> Vec<EntityOperation> {
        ...
        vec![set_entity_op("EthereumContractSource", id, entity)]
    }
}
...
impl EthereumContractMappingEntity {
    fn write_ops(self, id: &str) -> Vec<EntityOperation> {
        ...
        for (i, abi) in self.abis.into_iter().enumerate() {
            ...
            ops.append(&mut abi.write_ops(&abi_id));
        ...
        for (i, event_handler) in self.event_handlers.into_iter().enumerate() {
            ...
            ops.append(&mut event_handler.write_ops(&handler_id));
        ...
        ops.push(set_entity_op("EthereumContractMapping", id, entity));
...
impl EthereumContractAbiEntity {
    fn write_ops(self, id: &str) -> Vec<EntityOperation> {
        ...
        entity.insert("name".to_owned(), self.name.into());
        entity.insert("file".to_owned(), self.file.into());
        vec![set_entity_op("EthereumContractAbi", id, entity)]
    }
}
...
impl EthereumContractEventHandlerEntity {
    fn write_ops(self, id: &str) -> Vec<EntityOperation> {
        ...
        vec![set_entity_op("EthereumContractEventHandler", id, entity)]
    }
}
